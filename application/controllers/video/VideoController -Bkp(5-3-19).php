...
class VideoController extends CI_Controller{
    ...
    public function videoEdit() {
        ...
        $v_time = gmdate("H:i:s", $data['v_time']);
        ...
        $content = shell_exec('ffmpeg -i '.$this->config1['input_dir'].' -ss '.$s_time.' -t '.$v_time.' -async 1 -strict -2 '.$this->config1['output_dir']);

            
            // if (file_exists($this->config1['input_dir'])) unlink($this->config1['input_dir']);
            // delete_files($this->config1['input_dir_name'],true);
            // array_map('unlink', array_filter((array) glob($this->config1['input_dir_name']."/*")));
