...
<!--                                                <li style="border-left: 2px solid lightgray;">
                            ...
                            <div class="material-datatables" id="filter_data">
                                <table id="advertisementExpiredTable" class="table table-striped table-no-bordered table-hover dt-responsive" cellspacing="0" width="100%" style="width:100%">
                                    <thead>
                                        <tr>
  ...
  <script src="<?= base_url(); ?>theme/backend/image-crop/js/main.js"></script>
    ...
    <script>
        $(document).ready(function () {
            $('#advertisementExpiredTable').DataTable({
   ...
   $('#status_5').on('click', function(){
    ...
    function filter_get(){
         $.ajax({
            url:'<?= site_url('filter'); ?>',
            type:'post',
            data:{use:'expired',id:id_search,from:dateFrom,to:dateTo,location:location,status:status},
            success:function(data){
                $('#filter_data').empty();
...
$("#downloadXls").click(function(){
  $("#advertisementExpiredTable").table2excel({
    // exclude CSS class
    exclude: ".noExl",
    name: "Worksheet Name",
