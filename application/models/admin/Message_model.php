...
class Message_model extends CI_Model{
    ...
    public function get_chat() {
        $sql = "select DISTINCT a.*,c.*,a.ID as chatID,c.Country as Country,c.StatusID as stID,c.ID as user_id,max(a.NotifyAdmin) as NotifyAdmin_c from chat a "
                . "left join advertisement b on a.AdsID=b.ID "
                . "left join advertiser c on b.UserID=c.ID "
                . "left join chat_message d on d.UserID=c.ID where a.StatusID <>3 group by (user_id) order by d.CreatedDT desc";
...
//                . "left join chat_message d on d.UserID=c.ID where a.StatusID <>3 and d.UserID=$user_id";
        $sql = "SELECT DISTINCT chat.*,chat_message.UserID,chat_message.CreatedDT as msg_dt,advertisement.BusinessName,advertisement.CaptionLine,"
                ...
                . " WHERE chat_message.UserID=$user_id and chat.StatusID <>3 group by (chat_message.ChatID) order by chat.NotifyAdmin DESC";
