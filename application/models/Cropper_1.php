...
class Cropper extends CI_Model {
  ...
  public function setFile($file, $originalPath) {
    ...
    if ($errorCode === UPLOAD_ERR_OK) {
      ...
      if ($type) {
        ...
        $src = $originalPath . 'original-'.date('YmdHis').$extension;
  ...
  public function setDst($thumbPath) {
    $imageCropName = date('YmdHis') . '.jpg';
    $this ->dstThumb = $imageCropName;
    return $this ->dst = $thumbPath . $imageCropName;
  }  
  //this thumbnail image
  public function setDst2($thumb_view) {
    $imageCropName = date('YmdHis') . '.jpg';
    $this ->dstThumb = $imageCropName;
    return $this ->dst = $thumb_view . $imageCropName;
  }  
  // crop image  
...
//      $dst_img_h = 220;
  ...
  public function crop2($src, $dst, $data) {
    if (!empty($src) && !empty($dst) && !empty($data)) {
      switch ($this ->type) {
        case IMAGETYPE_GIF:
          $src_img = imagecreatefromgif($src);
